...
description: "Reference-only checklist pack of Emil Kowalski's (animations.dev) implementation-level design-engineering details — e.g. iOS 16px input-zoom, 44px tap targets, tabular-nums to prevent layout shift, sessionStorage intro-once, copy-as-markdown docs URLs. PASSIVE: do NOT auto-activate on general design, UI, animation, typography, or accessibility prompts — the 'impeccable' skill is the default driver for design work. Invoke ONLY when the user explicitly names this skill (emil / emil-design-engineering) or explicitly asks for Emil Kowalski / animations.dev guidance."
---
